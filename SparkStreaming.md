...
#### spark submit 运行简单的 word count 的spark streaming 程序
- 先运行如下命令，打开某个端口
- 运行官方的简单例子，具体代码可在github的spark项目中查看，该例子监听ip:port的输入，并以" "分割，进行单词统计
```
    spark-submit --master local[2] \
    --class org.apache.spark.examples.streaming.NetworkWordCount \
    --name NetworkWordCount \
    /zx/spark-2.2.1-bin-hadoop2.7/examples/jars/spark-examples_2.11-2.2.1.jar hadoop000 9999
```
- 例子中的代码如下
```
        if (args.length < 2) {
